...
df_stu %>%
  ...
  left_join(
    df_grades %>% 
      select(department_id, reservation_stu, reservation_fac) %>% 
      group_by(department_id) %>% 
      summarize_all(~ mean(., na.rm = TRUE))
  ) %>% 
  mutate_if(is.numeric, ~ round(. * 100, 2) %>% as.character() %>% str_c("%")) %>%
  select(department_id, state = state_india, stu_res_admin = stu_res_official, stu_res_sample = reservation_stu, fac_res_admin = fac_res_official, fac_res_sample = reservation_fac) %>% 
  xtable()
  
df_stu %>% count(ea_seats_reserved_students_other)
```
