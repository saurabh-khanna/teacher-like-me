...
bal1 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>% 
  lm_robust(reservation_fac ~ reservation_stu, data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal2 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_associate_professor ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal3 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_professor ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal4 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_yearsinhighed ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal5 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_highest_degree_phd ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal6 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_highest_degree_phd_in_prog ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal7 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_degree_college_elite ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

bal8 <-
  df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_female ~ reservation_stu , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid)

knitreg(list(bal1, bal2, bal3, bal4, bal5, bal6, bal7, bal8), custom.note = "%stars \n All models control for course fixed effects. Standard errors clustered at faculty level.", stars = c(0.01, 0.05, 0.1), include.ci = FALSE, custom.model.names = c("Reservation", "Assoc Professor", "Professor", "Experience years", "Highest degree PhD", "Highest Degree PhD in Progress", "Degree college elite", "Female"), dcolumn = TRUE, booktabs = TRUE, float.pos = "h", caption = "", digits = 3)
```
