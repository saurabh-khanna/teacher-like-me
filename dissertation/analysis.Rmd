...
df_grades %>% 
  count(department_id, course_name, facid) %>% skim(n)
  #count(n >= 100)
  ggplot(aes(n)) +
  geom_histogram(aplha = 0.85) + 
  theme_minimal() +
  labs(x = "Students", y = "Count")
```
...
df_stu %>% 
  ...
  group_by(elite) %>%
  summarise(
    reservation_stu_perc_weighted = round(weighted.mean(reservation, w = sw_f, na.rm = T) * 100, 2),
    reservation_stu_perc_unweighted = round(mean(reservation, na.rm = T) * 100, 2),
...
facdata_mentions %>%
  distinct(facid, .keep_all = T) %>% 
  left_join(read_csv("/Users/saurabh/Everything/GitHub/teacher-like-me/data/depweights.csv"), by = "department_id") %>%
  left_join(df_stu %>% distinct(department_id, elite), by = "department_id") %>%
  group_by(elite) %>% 
  summarise(
    reservation_fac_perc_weighted = round(weighted.mean(fac_reservation, w = sw_f, na.rm = T), 2),
    reservation_fac_perc_unweighted = round(mean(fac_reservation, na.rm = T), 2),
    n_observations = sum(n())
  )
```
