...
## Table 6a: Regressions for Weekly Hours on Various Teaching-Related Activities
...
## Table 6b: Regressions for TPI Measures on Use of Teaching Practices
...
texreg(list(lm_tpi3, lm_tpi4, lm_tpi5, lm_tpi8), custom.note = "%stars", stars = c(0.01, 0.05, 0.1), include.ci = FALSE, dcolumn = TRUE, booktabs = TRUE, float.pos = "H", caption = "", digits = 2, custom.model.names = c("In-class features and activities", "Assignments", "Feedback and testing", "Collaboration"), custom.coef.names = c("Reservation faculty", "Highest degree is PhD", "Highest degree is PhD in progress", "Degree college elite", "Associate professor", "Professor", "Experience in years", "Female"))
```
...
## Table 8: Regressions for Number of Publications per Year
...
lm_pub2 <-
  ...
  left_join(read_dta(here::here("data/fac_covariates.dta")) %>% select(facid, fac_publications), by = "facid") %>% 
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_publications ~ reservation_fac + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite, data = ., fixed_effects = ~ stdid + fe_var, se_type = "stata", clusters = facid)

lm_pub3 <-
  df_mechanism %>% 
  left_join(read_dta(here::here("data/fac_covariates.dta")) %>% select(facid, fac_publications), by = "facid") %>% 
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_publications ~ reservation_fac + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_associate_professor + fac_professor + fac_yearsinhighed, data = ., fixed_effects = ~ stdid + fe_var, se_type = "stata", clusters = facid)

lm_pub4 <-
  df_mechanism %>% 
  left_join(read_dta(here::here("data/fac_covariates.dta")) %>% select(facid, fac_publications), by = "facid") %>% 
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  lm_robust(fac_publications ~ reservation_fac + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_female, data = ., fixed_effects = ~ stdid + fe_var, se_type = "stata", clusters = facid)

texreg(list(lm_pub1, lm_pub2, lm_pub3, lm_pub4), custom.note = "%stars", stars = c(0.01, 0.05, 0.1), include.ci = FALSE, dcolumn = TRUE, booktabs = TRUE, float.pos = "H", caption = "", digits = 2, custom.model.names = c("I", "II", "III", "IV"), custom.coef.names = c("Reservation faculty", "Highest degree is PhD", "Highest degree is PhD in progress", "Degree college elite", "Associate professor", "Professor", "Experience in years", "Female"))
```

```{r, eval=F}
df_mechanism %>% 
  distinct(facid, .keep_all = F) %>% 
  left_join(read_dta(here::here("data/fac_covariates.dta")) %>% select(facid, fac_publications), by = "facid") %>% 
  summary()
```
...
\toprule
 & \multicolumn{1}{c}{I} & \multicolumn{1}{c}{II} & \multicolumn{1}{c}{III} & \multicolumn{1}{c}{IV} \\
\midrule
Reservation faculty               & -3.31^{**} & -1.41       & -1.71       & -1.72       \\
                                  & (1.46)     & (1.27)      & (1.28)      & (1.28)      \\
Highest degree is PhD             &            & 12.35^{***} & 5.17^{*}    & 5.34^{*}    \\
                                  &            & (2.31)      & (2.84)      & (2.85)      \\
Highest degree is PhD in progress &            & 2.66^{**}   & 2.80^{*}    & 2.98^{**}   \\
                                  &            & (1.18)      & (1.43)      & (1.44)      \\
Degree college elite              &            & 1.21        & 1.38        & 1.43        \\
                                  &            & (1.47)      & (1.49)      & (1.50)      \\
Associate professor               &            &             & 7.22^{***}  & 7.21^{***}  \\
                                  &            &             & (1.75)      & (1.74)      \\
Professor                         &            &             & 16.30^{***} & 16.10^{***} \\
                                  &            &             & (4.27)      & (4.25)      \\
Experience in years               &            &             & -0.32^{**}  & -0.32^{**}  \\
                                  &            &             & (0.14)      & (0.14)      \\
Female                            &            &             &             & -1.02       \\
                                  &            &             &             & (1.08)      \\
\midrule
Num. obs.                         & 38021      & 38021       & 37970       & 37970       \\
N Clusters                        & 499        & 499         & 497         & 497         \\
\midrule
...
df_stu %>% 
  ...
  select(reservation, female, age, ses, father_college, mother_college) %>% 
  bind_rows(
    df_grades %>% distinct(stdid, .keep_all = T) %>% select(reservation = reservation_stu, female, age, ses, father_college, mother_college), .id = "analysis_sample"
  ) %>% 
  mutate(
    analysis_sample = as.integer(analysis_sample) - 1L,
