...
df_grades <-
  ...
  filter(facid != "") %>%
  left_join(df_stu %>% select(stdid, department_id, reservation, took_jee, b_i_jeemain_score, b_math_g1_score, b_physics_g1_score, e_math_g3_score, e_physics_g3_score, elite, b_ct_score, b_ql_score, b_rr_score), by = "stdid") %>%
...
df_grades %>%
  mutate(fe_var = str_c(department_id, course_name, sep = "_")) %>%
  mutate(depscore = scale(b_math_g1_score) %>% as.vector) %>%
  lm_robust(depscore ~ reservation_fac , data = ., fixed_effects = ~ fe_var, se_type = "stata", clusters = facid) %>%
  knitreg(include.ci = F, digits = 3, stars = c(0.01, 0.05, 0.1))
```
