...
df_grades <-
  ...
  filter(facid != "") %>%
  left_join(df_stu %>% select(stdid, department_id, reservation, caste, b_i_jeemain_score, b_math_g1_score, b_physics_g1_score, e_math_g3_score, e_physics_g3_score, b_ct_score, b_ql_score, e_ct_score, e_ql_score, b_rr_score, e_rr_score, e_attend_grad_school, e_college_satisfied, e_dropped_out, e_math_attend = e_univtimeuse_1, e_physics_attend = e_univtimeuse_4, e_hot_attend = e_univexpr_55), by = "stdid") %>%
...
The dependent variables are standardized test scores. All models include baseline score controls and department fixed effects.
...
The dependent variable is daily hours attending classes (0-10).
...
df_temp <-
  ...
  arrange(stdid, subject) %>% 
  relocate(stdid, subject) %>% 
  mutate(e_attend = if_else(e_attend > 10, 10, e_attend)) %>%
  mutate(subject = as_factor(str_to_title(subject)))
...
lm2 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_attend ~ reservation_fac + subject + female + age + miss_age + father_college + mother_college, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm3 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_attend ~ reservation_fac + subject + female + age + miss_age + father_college + mother_college + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_female, data = ., se_type = "stata", fixed_effects = ~ department_id)
...
df_temp <-
  ...
  drop_na(e_hot_attend) %>%
  group_by(stdid, reservation_stu, female, father_college, mother_college, e_hot_attend, age, miss_age, department_id) %>%
  summarise(
    reservation_fac = mean(reservation_fac, na.rm = T),
    fac_associate_professor = mean(fac_associate_professor, na.rm = T),
    fac_professor = mean(fac_professor, na.rm = T),
  ...
  ungroup() %>% 
  mutate(e_hot_attend = if_else(e_hot_attend > 10, 10, e_hot_attend)) %>% 
  relocate(stdid, reservation_fac, e_hot_attend)
```
...
lm1 <-
  df_temp %>%
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_hot_attend ~ reservation_fac, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm2 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_hot_attend ~ reservation_fac + female + age + miss_age + father_college + mother_college, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm3 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_hot_attend ~ reservation_fac + female + age + miss_age + father_college + mother_college + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_female, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm4 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_hot_attend ~ reservation_fac * reservation_stu + female + age + miss_age + father_college + mother_college + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_female, data = ., se_type = "stata", fixed_effects = ~ department_id)
...
The dependent variable is college satisfaction (0/1). All models include department fixed effects.
...
The dependent variable is plans for grad school (0/1). All models include department fixed effects.
...
The dependent variable is college dropout (0/1). All models include department fixed effects.
...
read_dta(here::here("data/30jan2022/semesters.dta")) %>% 
  inner_join(df_grades %>% distinct(facid)) %>%
