...
The dependent variable is weekly hours attending classes (0-30).
...
df_temp <-
  ...
  mutate(
    e_attend = e_attend * 5,
    e_attend = if_else(e_attend > 30, 30, e_attend)
  ) %>%
  mutate(subject = as_factor(str_to_title(subject)))
...
df_temp <-
  ...
  ungroup() %>%
  mutate(
    e_hot_attend = e_hot_attend * 5,
    e_hot_attend = if_else(e_hot_attend > 30, 30, e_hot_attend)
  ) %>%
...
lm2 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_hot_attend ~ reservation_fac + reservation_stu + female + age + miss_age + father_college + mother_college, data = ., se_type = "stata", fixed_effects = ~ department_id)
...
The dependent variable growth mindset is created by applying the Anderson index to 3 survey items on intelligence beliefs. This variable is then turned into a z-score before use in below regression models.
...
Dependent variable is receiving paid or unpaid tutoring (0/1). We combine data from HOT and Math surveys and use survey type as a control in the regression models.
...
Dependent variable is talking to faculty about research (0/1). We combine data from HOT and Math surveys and use survey type as a control in the regression models.
...
df_math <-
  ...
  drop_na(e_math_g3_score, e_talkprof_research) %>%
  group_by(stdid, reservation_stu, female, father_college, mother_college, e_talkprof_research, age, miss_age, department_id) %>% 
  summarise(
    reservation_fac = mean(reservation_fac, na.rm = T),
    fac_associate_professor = mean(fac_associate_professor, na.rm = T),
    fac_professor = mean(fac_professor, na.rm = T),
  ...
  ungroup() %>% 
  mutate(survey = "math") %>%
  relocate(stdid, reservation_fac, e_talkprof_research)
...
lm1 <-
  df_temp %>%
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_talkprof_research ~ reservation_fac + survey, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm2 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_talkprof_research ~ reservation_fac + survey + reservation_stu + female + age + miss_age + father_college + mother_college, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm3 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_talkprof_research ~ reservation_fac + survey + reservation_stu + female + age + miss_age + father_college + mother_college + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_female, data = ., se_type = "stata", fixed_effects = ~ department_id)

lm4 <-
  df_temp %>% 
  drop_na(female, father_college, mother_college) %>%
  lm_robust(e_talkprof_research ~ reservation_fac * reservation_stu + survey + female + age + miss_age + father_college + mother_college + fac_associate_professor + fac_professor + fac_yearsinhighed + fac_highest_degree_phd + fac_highest_degree_phd_in_prog + fac_degree_college_elite + fac_female, data = ., se_type = "stata", fixed_effects = ~ department_id)
...
Dependent variable is working with faculty on their research (0/1). We combine data from HOT and Math surveys and use survey type as a control in the regression models.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lllllll@{}}
    ...
    Baseline CT score                 &           & 6.62          & 3.04        &           & 0.239*                      & 0.126       \\
    Baseline QL score                 &           & 9.00          & 4.57        &           & 0.393**                     & 0.169       \\
    Baseline RR score                 &           & 13.61         & 4.88        &           & 0.261*                      & 0.154       \\
